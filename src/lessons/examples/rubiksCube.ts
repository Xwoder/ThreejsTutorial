    ...
    for (let y = -1; y <= 1; y++) {
      for (let z = -1; z <= 1; z++) {
        ...
        const materials = [
          ...
          new THREE.MeshStandardMaterial({ color: y === 1 ? FACE_COLORS.up : INNER_COLOR }),
          new THREE.MeshStandardMaterial({ color: y === -1 ? FACE_COLORS.down : INNER_COLOR }),
...
export const rubiksCube: Lesson = {
  ...
  create(container) {
    ...
    function faceMove(face: string, reverse: boolean) {
      ...
      switch (face) {
        case 'U': rotateLayer('y', 1, amount); break;
        case 'D': rotateLayer('y', -1, -amount); break;
        case 'L': rotateLayer('x', -1, amount); break;
        case 'R': rotateLayer('x', 1, -amount); break;
        case 'F': rotateLayer('z', 1, -amount); break;
        case 'B': rotateLayer('z', -1, amount); break;
      }
    }
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        case 'u': faceMove('U', reverse); break;
        case 'd': faceMove('D', reverse); break;
        case 'l': faceMove('L', reverse); break;
        case 'r': faceMove('R', reverse); break;
        case 'f': faceMove('F', reverse); break;
        case 'b': faceMove('B', reverse); break;
